-- Re-oferta: se a oferta EXPIROU (entregador não viu a tempo) e o pedido segue buscando,
-- o sistema volta a ofertar pra ele. Antes, qualquer oferta anterior (mesmo expirada)
-- excluía o entregador pra sempre — com 1 entregador online, o pedido travava buscando.
-- Continua NÃO re-ofertando pra quem RECUSOU de propósito (nem quem tem oferta ativa/aceitou).
...
begin
  ...
  where e.is_online = true
    ...
    and not exists (select 1 from ofertas o where o.pedido_id = p_pedido_id and o.entregador_id = e.id
                     and (o.status in ('recusada','aceita') or (o.status = 'ofertada' and o.expira_at > now())))
