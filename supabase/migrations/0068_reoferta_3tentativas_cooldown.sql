-- Política de re-oferta (regra do dono): o sistema PODE mandar a mesma corrida pro mesmo
-- entregador mesmo que ele tenha recusado. Até 3 tentativas seguidas; depois, cooldown de
-- 2 minutos entre as próximas tentativas. Se ele for o único online, continua recebendo.
-- Nunca re-oferta se ele JÁ ACEITOU, e nunca dispara enquanto ele tem uma oferta ATIVA.
...
begin
  ...
  where e.is_online = true
    ...
    -- nunca se já aceitou; nunca enquanto tem oferta ativa (dele em qualquer pedido)
    and not exists (select 1 from ofertas o where o.pedido_id = p_pedido_id and o.entregador_id = e.id and o.status = 'aceita')
    ...
    -- até 3 tentativas seguidas; depois só a cada 2 min
    and (
      (select count(*) from ofertas o where o.pedido_id = p_pedido_id and o.entregador_id = e.id) < 3
      or (select max(o.ofertada_at) from ofertas o where o.pedido_id = p_pedido_id and o.entregador_id = e.id) <= now() - interval '2 minutes'
    )
  ...
  -- 2) FALLBACK: ninguém no raio → o mais próximo disponível (mesmas regras)
