...
export default function ComunicadosNegocio() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!sb) return;
      // RLS comunicados_leitura: lojista recebe 'todos' + 'negocios'
  ...
  return (
    <NegocioShell title="Comunicados">
      <div className="card">
        <div className="card-h"><Icon name="send" /><h3>Avisos da plataforma</h3><span className="right">{coms.length}</span></div>
        {carregando ? (
          <div style={{ fontSize: 12.5, color: "var(--faint)" }}>Carregando…</div>
        ) : coms.length === 0 ? (
          <div style={{ fontSize: 12.5, color: "var(--faint)" }}>Nenhum aviso por enquanto. Comunicados da operação aparecem aqui.</div>
        ) : (
          coms.map((c) => (
