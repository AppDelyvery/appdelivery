...
export default function AvaliacoesEntregador() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [aR, eR] = await Promise.all([
        sb.rpc("minhas_avaliacoes"),
        sb.from("entregadores").select("rating").limit(1).maybeSingle(),
      ]);
  ...
  return (
    <EntregadorShell title="Avaliações">
      <div className="card" style={{ textAlign: "center" }}>
        ...
        <div style={{ fontSize: 12.5, color: "var(--muted)", marginTop: 6 }}>{total} avaliação(ões) de clientes</div>
      ...
      <div className="card">
        ...
        {carregando ? (
          ...
          <div style={{ fontSize: 12.5, color: "var(--faint)" }}>Ainda sem comentários. Capriche nas entregas que eles vêm!</div>
        ) : (
          comentarios.map((a, i) => (
