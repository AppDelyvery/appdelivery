...
export default function GanhosEntregador() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!sb) return;
      // RLS pedidos_party_sel: entregador lê os pedidos em que é o designado
      const { data } = await sb
        .from("pedidos")
  ...
  return (
    <EntregadorShell title="Ganhos">
      ...
      <div className="card">
        ...
        <p className="hint">Você recebe 80% do frete de cada entrega concluída. Repasse automático liga com o Asaas (split na conclusão).</p>
      </div>
    </EntregadorShell>
  );
}
